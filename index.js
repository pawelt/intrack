...
`;h(J,({changes:e})=>{if(re()==="instruments")for(let t of e){let r=_r(t.instrument.code);switch(t.op){case"create":H(_e(),n(xt,{instrument:t.instrument}));break;case"update":G(r,n(xt,{instrument:t.instrument}));break;case"delete":r?.remove();break}}});var Hr=()=>He().value="",Bn=h(S,Hr),Xn=h(S,async()=>{gt().disabled=!0;try{await Gt(He().value)}catch(e){alert(e)}gt().disabled=!1,Hr()}),jn=h(S,async({xid:e=""})=>{let{name:t}=F()[e]||{};!t||!confirm(`Remove instrument:  
${t} 
? 
It will NOT be removed from wallets (you will need to do it manually)`)||await Xt(e)}),qn=()=>n("div",null,"Paste the URL of the instrument you want to track. Supported websites:",n("ul",null,n("li",null,n("a",{href:"https://live.euronext.com/en"},"oslobors funds")," ",n("small",null,"( for example:"," ",n("a",{href:"https://live.euronext.com/en/product/funds/NO0010748155.ODEIEND-WOMF",target:"_blank"},"https://live.euronext.com/en/product/funds/NO0010748155.ODEIEND-WOMF")," ",")")),n("li",null,n("a",{href:"https://live.euronext.com/en"},"oslobors stocks")," ",n("small",null,"( for example:"," ",n("a",{href:"https://live.euronext.com/en/product/equities/NO0010823131-MERK",target:"_blank"},"https://live.euronext.com/en/product/equities/NO0010823131-MERK")," ",")")))),Gn=()=>n("div",null,n("input",{xname:He}),"  ",n(R,{xclick:Xn,xname:gt},"Add instrument"),"  ",n(R,{xclick:Bn,variant:"text"},"Clear")),xt=({instrument:e})=>n("tr",{xname:_r,xid:e.code},n("td",null,n("a",{href:e.sourceUrl,target:"_blank"},"(",e.type,") ",n("strong",null,e.name))),n("td",{class:"right"},n("strong",null,e.latestPrice.toFixed(2))),n("td",null,e.code),n("td",null,e.isin),n("td",{class:"updatedAgo","data-latest-update":e.latestUpdate},be(e.latestUpdate)),n("td",null,n(R,{xclick:jn,xid:e.code,variant:"text"},"Delete"))),Fr=()=>{let e=n("div",{class:zn},n(qn,null),n(Gn,null),n("h2",{class:"title"},"Tracked instruments"),n("table",{xname:_e},ve().map(t=>n(xt,{instrument:t}))));return e.onMount=()=>{zr(),console.log("ViewInstruments mounted!")},e},zr=()=>{re()==="instruments"&&(_e().querySelectorAll("[data-latest-update]").forEach(e=>e.innerHTML=be(e.dataset.latestUpdate||"")),setTimeout(zr,5e3))};var Fe=k("new-wallet-name"),yt=k("wallet-list"),ne=K("wallet-block"),vt=K("wallet-title"),wt=K("wallet-total"),bt=K("wallet-instruments"),St=K("updated-ago"),Br=j("wallet-instrument-list"),Tt=j("wallet-new-total-price"),It=j("wallet-new-unit-price"),Mt=j("wallet-new-date"),kt=j("wallet-new-instrument"),ke={},Kn=U`
  -label: view-wallets;
...
`,Xr=()=>Fe().value="",jr=e=>{Tt(e).value="",It(e).value="",Mt(e).value=nt(new Date),kt(e).value=""},qr=e=>{G(wt(e.name),n(Gr,{wd:e})),G(vt(e.name),n(Kr,{wd:e}))};h(J,({changes:e=[]}={})=>{if(re()!=="wallets")return;let t=e.filter(r=>r.op==="update");if(!!t.length)for(let r of Le()){let o=r.instruments.filter(i=>t.some(c=>c.instrument.code===i.code));if(!o.length)continue;let a=Re(r),s=ne(r.name);o.forEach(i=>{let c=a.instruments.find(l=>l.id==i.id);c&&G(xe(`${r.name}:${i.id}`)(s),n(Nt,{ins:c,walletName:a.name}))}),qr(a)}});h(st,({wallet:e,modifiedInstrumentId:t})=>{let r=Re(e),o=ne(e.name);r.instruments.some(a=>a.id===t)?(H(Br(o),n(Nt,{ins:r.instruments.slice(-1)[0],walletName:r.name})),jr(o)):t&&xe(`${e.name}:${t}`)(o).remove(),qr(r)});h(at,({wallet:e})=>{ke[e.name]=!0,Xr(),H(yt(),n(Yr,{wallet:e}))});h(it,({name:e})=>ne(e)?.remove());var Yn=h(S,({xid:e=""})=>{ke[e]=!ke[e],bt(e).classList.toggle("expanded",ke[e])}),Jn=h(S,Xr),Qn=h(S,({xid:e=""})=>jr(ne(e))),Zn=h(S,async()=>{let e=Fe().value.trim();!e||await Yt({name:e,comment:"",instruments:[]})}),eo=h(S,async({xid:e=""})=>{!oe()[e]||!confirm(`Delete wallet:  ${e} 
...
`,sn=()=>{let[,...e]=Object.entries(Xe);return n("div",{class:fo},n(ht,{href:location.pathname},n("h1",null,"Investment tracker")),e.map(([t,r],o)=>n(u,null,o>0?" | ":"",n(ht,{view:t},r.label))))};Dr`
  html, body {
    height: 100vh;
  }
  body {
...
`;var cn=k("current-view");h(Ke,()=>{let{Component:e}=an();Ce(cn(),n(e,null))});var ln=()=>{let e=n("div",{class:"app"},n(sn,null),n("hr",null),n("div",{xname:cn}));return e.onMount=()=>{console.log("App mounted!")},e};Promise.all([jt(),Qt()]).then(()=>{we(),Rt(),G(document.getElementById("app"),n(ln,null)),Pt()});})();
